 ...
 * a string. */
	...
	//Autoranging
	if (fabs(outputValue) >= 1 && unit!=(char)222) //Fabs gives absolute value of float, polarity of number is irrelevant 
	{
		snprintf (stringOut, 10, "%.3f", outputValue);
	}
	else if (unit!=(char)222)
	{
...
char* ohmFunc(float volts){
	...
	if (ohmMode==0)
	{
		strcpy(rUnit,(char)222);
	}
	else
	{
		strcpy(rUnit, "k");
		strcat(rUnit,(char)222);
	}
	return rUnit;
}
int main (void) {
