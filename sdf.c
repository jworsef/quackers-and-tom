 ...
 * a string. */
	...
	{
		outputValue = ((volts - 1.5f)/0.15f - 0.2f)/10.05f - 0.0085f;
	}
	else if (unit == (char)222) //Ohms
	{
		outputValue = (((volts - 1.5f)/0.15f - 0.2f)*995.8f)/(5-((volts - 1.5f)/0.15f - 0.2f));
	}
 ...
 * Only performs string and LCD operations, all convertions and calculates happen in arvStringfromValue*/
	...
	if((voltsADC<0.1f || voltsADC>2.95f) && myFlag==0)
	{
	...
	if((0.2f<voltsADC && voltsADC<2.9f) && myFlag==1) 
	{
		//set the flag low
		myFlag=0;
	}
...
void Rfunc(struct ValuesList* list)
{
	...
	/*	if (ohmMode==CurrentOhmMode)
		...
		volts =(((valueGivenByADC() - 1.5f)/0.15f - 0.2f)*995.8f)/(5-((valueGivenByADC() - 1.5f)/0.15f - 0.2f));//(valueGivenByADC()*995.8f)/(5-valueGivenByADC());
		if (rFlag==1)
		{
			rFlag=0;
			waitForSeconds(2);
		...
		if (refreshDisplay==1)
		{
			...
			if (ohmMode == 0)
			{
				...
				if (sahOn == 1)
				{
					snprintf(LCD_minmax, 15, "%0.1f%c %0.1f%c", voltsSaHMin,(char)222,voltsSaHMax,(char)222);
