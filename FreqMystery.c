...
char*	arvStringfromValue(float volts, char unit, int minmax){
 ...
 * a string. */
	...
	if (unit == 'z'){
		snprintf (stringOut, 10, "%0.0f", outputValue);
		strcat(stringOut, "H");
	}
 ...
 * Only performs string and LCD operations, all convertions and calculates happen in arvStringfromValue*/
	...
	strcpy(LCD_out, arvStringfromValue(voltsADC, unit,0));
	...
	if (sahOn == 1)
	{
		strcpy(LCD_minmax, arvStringfromValue(voltsSaHMin, unit,1));
		strncat(LCD_minmax, arvStringfromValue(voltsSaHMax, unit,1), 16);
		PB_LCD_GoToXY(0, 1);
	...
	/*	if (ohmMode==CurrentOhmMode)
		...
		if (refreshDisplay==1)
		{
			...
			if (ohmMode == 0)
			{
				...
				if (sahOn == 1)
				{
					snprintf(LCD_minmax, 16, "%0.3f%c %0.3f%c", voltsSaHMin,(char)222,voltsSaHMax,(char)222);
			...
			else
			{
				...
				if (sahOn == 1)
				{
					snprintf(LCD_minmax, 16, "%0.2fK%c %0.2fK%c", voltsSaHMin,(char)222,voltsSaHMax,(char)222);
